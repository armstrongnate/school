...
fun il2rl lst = map (fn i => real i) lst
...
fun squarelist lst = map (fn i => i*i) lst
...
fun inclist lst i = map (fn n => n + i) lst
...
fun sqsum lst = foldl (fn (a,b) => b + a*a) 0 lst
...
fun bor lst = foldl (fn (a,b) => b orelse a) false lst
...
fun band lst = foldl (fn (a,b) => b andalso a) true lst
...
fun bxor lst = foldl (fn (a,b) => (a orelse b) andalso not(a andalso b)) false
  lst
...
fun duplist lst = foldl (fn (a,b) => b @ [a,a]) [] lst
...
fun mylength lst = foldl (fn (_,a) => a+1) 0 lst
...
fun il2absrl lst = map (fn i => real (abs i)) lst
...
fun truecount lst = foldl (fn (b,a) => if b then a+1 else a) 0 lst
...
fun myimplode chars = foldl (fn (c, a) => a^(str c)) "" chars
...
fun lconcat lsts = foldl (fn (l, a) => a @ l) [] lsts
...
fun append lst1 = fn lst2 => foldr (fn (b,a) => b::a) lst2 lst1
...
fun less (e, lst) = foldl (fn (b, a) => if b<e then a @ [b] else a) [] lst
...
fun evens lst = foldl (fn (b, a) => if (b mod 2) = 0 then a @ [b] else a) [] lst
...
fun convert (lst: ('a * 'b) list) = (foldl (fn (b,a) => a@[#1 b]) [] lst, foldl (fn (b,a) => a@[#2 b]) [] lst)
...
fun mymap f = fn lst => foldr (fn (b, a) => (f b) :: a) [] lst
...
fun eval lst = fn x => foldr (fn (c, a) => x * a + c) 0.0 lst
